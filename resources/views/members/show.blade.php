...
<div style="display:grid;grid-template-columns:300px 1fr;gap:20px;align-items:start;">
  ...
  <div style="display:flex;flex-direction:column;gap:16px;">
    ...
    {{-- Profile Card --}}
    <div style="background:var(--surface1);border:1px solid var(--border);
      ...
      {{-- Status Badge --}}
      @php
        // ✅ Use dynamic accessor — never read raw DB status column
        $dynamicStatus = $member->status; // calls getStatusAttribute() / Attribute::make()
        $end           = $member->end_date; // already cast to Carbon via $casts
        $daysLeft      = $end ? (int) now()->diffInDays($end, false) : null;

        // Map dynamic status → badge appearance
        match ($dynamicStatus) {
            'Active' => [
                $badgeLabel  = 'Active',
                $badgeColor  = '#4ade80',
    ...
    <div style="background:var(--surface1);border:1px solid var(--border);
      ...
      <div style="display:flex;align-items:center;gap:14px;padding:14px;
        ...
        <div>
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:3px;">Phone</div>
      ...
      <div style="display:flex;align-items:center;gap:14px;padding:14px;
        ...
        <div>
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:3px;">Gender</div>
          <div style="font-size:14px;font-weight:600;color:var(--text);">
            {{ $member->gender ?? '—' }}
          </div>
        </div>
      ...
      <div style="display:flex;align-items:center;gap:14px;padding:14px;
        ...
        <div>
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:3px;">Birthdate</div>
          <div style="font-size:14px;font-weight:600;color:var(--text);">
      ...
      <div style="display:flex;align-items:center;gap:14px;padding:14px;
        ...
        <div>
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:3px;">Address</div>
          <div style="font-size:14px;font-weight:600;color:var(--text);">
            {{ $member->address ?? '—' }}
          </div>
        </div>
    ...
    <div style="background:var(--surface1);border:1px solid var(--border);
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
          ...
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:10px;">Fitness Plan</div>
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
                    border-radius:12px;padding:18px 20px;">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:10px;">Duration</div>
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
                    border-radius:12px;padding:18px 20px;">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:10px;">Fee</div>
          <div style="font-size:18px;font-weight:700;color:var(--accent);">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:20px;">
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
                    border-radius:12px;padding:18px 20px;">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:10px;">Start Date</div>
          <div style="font-size:18px;font-weight:700;color:var(--text);">
            {{ $member->start_date?->format('M d, Y') ?? '—' }}
          </div>
        </div>
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
                    border-radius:12px;padding:18px 20px;">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:10px;">End Date</div>
          {{-- Color end date based on dynamic status --}}
          @php
          ...
          @endphp
          <div style="font-size:18px;font-weight:700;color:{{ $endDateColor }};">
            {{ $end?->format('M d, Y') ?? '—' }}
          </div>
        </div>
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
                    border-radius:12px;padding:18px 20px;">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;
                      color:var(--muted);margin-bottom:10px;">Status</div>
          <div style="display:inline-flex;align-items:center;gap:7px;
                      font-size:16px;font-weight:700;color:{{ $badgeColor }};">
            <span style="width:7px;height:7px;border-radius:50%;background:currentColor;
      ...
      @if($end && !$end->isPast())
        ...
        @endphp
        <div style="padding:16px 20px;background:var(--surface2);border:1px solid var(--border);
                    border-radius:10px;margin-bottom:20px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
            <div style="font-size:10px;color:var(--muted);text-transform:uppercase;
                        letter-spacing:1px;font-weight:700;">Days Remaining</div>
          ...
          <div style="background:var(--border);border-radius:999px;height:6px;overflow:hidden;">
            <div style="height:100%;border-radius:999px;width:{{ $progressPct }}%;
                        background:{{ $barColor }};transition:width 0.4s ease;"></div>
          </div>
        </div>
      @endif
      ...
      @if($dynamicStatus === 'Expiring Soon')
        <div style="display:flex;align-items:center;gap:12px;padding:16px 20px;
                    ...
                    border-radius:10px;color:#fbbf24;font-size:14px;font-weight:500;">
